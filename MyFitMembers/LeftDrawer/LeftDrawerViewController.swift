...
class LeftDrawerViewController: UIViewController,DelegateClientCollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        setUpTableView()
        NotificationCenter.default.addObserver(self, selector: #selector(LeftDrawerViewController.setUpTableView), name: NSNotification.Name(rawValue: "ConfigureLeftDrawer"), object: nil)
    }
    ...
    func setUpTableView(){
        ...
        guard let imageURL = URL(string: ApiCollection.apiImageBaseUrl + imageUrl) else {return}
        
        btnUserImage.yy_setImage(with: imageURL, for: UIControlState(), options: .progressiveBlur)
...
extension LeftDrawerViewController {
    ...
    func getFitnessAssessment(_ vc : UIViewController){
        ...
        ApiDetector.getDataOfURL(ApiCollection.apiWeeklyAssessmentOfClient, dictForBackend: dictForBackEnd, failure: { (data) in
            print(data)
                ...
                if let navVC = (UIApplication.shared.delegate as? AppDelegate)?.window?.rootViewController as? LeftNavigationViewController,let VC = navVC.topViewController {
                    VC.pushVC(vc)
                }
            }, method: .post, viewControl: self, pic: UIImage(), placeHolderImageName: "", headers: [:] , showLoader: true, loaderColor: Colorss.darkRed.toHex())
    }
    
    func getWeighIn(_ vc : UIViewController){
        let dictForBackEnd = API.ApiCreateDictionary.clientListing().formatParameters()
    ...
    func getClientMeasurements(_ vc : UIViewController){
        let dictForBackEnd = API.ApiCreateDictionary.clientListing().formatParameters()
    ...
    func pushContentVc(_ vc : UIViewController)  {
        if let navVC = (UIApplication.shared.delegate as? AppDelegate)?.window?.rootViewController as? LeftNavigationViewController,let VC = navVC.topViewController {
    ...
    @IBAction func btnActionProfileButton(_ sender: UIButton) {
        guard let isClient = UserDataSingleton.sharedInstance.loggedInUser?.isClient  else {return}
