...
class FitnessAssesment : NSObject {
    ...
    init(attributes : OptionalSwiftJSONParameters){
        ...
        fitnessAssesment?.forEach({ (index, element) in
            if index == 0{
                self.feedData = Array(repeating: [], count: element.fitnessAssessments.count)
                self.feedData?[index] = []
            }else{}
...
//            }
//        })
        self.fitnessDates = Array(repeating: "", count: fitnessAssesment?.count ?? 0)
...
class WeeklyFitnessAssessment: NSObject , JSONDecodable {
    ...
    class func changeDictionaryToUserArray(_ array1 : [JSON])->[WeeklyFitnessAssessment]{
