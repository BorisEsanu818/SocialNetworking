...
//  MyFitMembers
//
//  Created by Boris Esanu on 28/10/16.
...
class FitnessAssesment1 : NSObject {
    ...
    init(attributes : OptionalSwiftJSONParameters){
        ...
        fitnessAssesment1?.forEach({ (index, element) in
            if index == 0{
                self.feedData1 = Array(repeating: [], count: element.fitnessAssessments1.count)
...
//        })
        ...
        self.fitnessDates1 = Array(repeating: "", count: fitnessAssesment1?.count ?? 0)
...
class WeeklyFitnessAssessment1: NSObject , JSONDecodable {
    ...
    class func changeDictionaryToUserArray(_ array1 : [JSON])->[WeeklyFitnessAssessment1]{
